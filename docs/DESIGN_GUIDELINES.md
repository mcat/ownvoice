...
- **Pinch-to-zoom is disabled in the installed PWA.** The viewport meta sets `user-scalable=no, maximum-scale=1`. This is a deliberate deviation from WCAG 1.4.4 (Resize text). The justification: every patient-facing target is already at 64×64 px or larger, the typography baseline is 18 px, and the app ships its own font-scale slider in Settings — so the platform-level pinch-zoom is redundant and (worse) creates accidental zooms during tremor or palm contact. iOS only honors this directive inside an installed PWA; browsing on the web in Safari proper still allows zoom.
- **The system long-press menu (callout, magnifier, copy/share) is suppressed app-wide** via `-webkit-touch-callout: none` and `user-select: none` on `html, body`. Inputs, contenteditable surfaces, and any element marked `data-selectable="true"` opt back in — this is how the Diagnostics activity log keeps text-copy for clinicians while phrase tiles stay clean.
