...
describe("audioCache — generateAllPhrases gpuOnly", () => {
  ...
  it("cache hits reset the consecutive-failure counter (no premature breaker trip)", async () => {
    ...
    // TWO places — on cache hit (outcome === null) and on successful
    // synth. The "resets on a successful phrase" test below covers the
    // synth path; this one covers the cache-hit path, which a targeted
    // mutant could remove without affecting any other test.
    //
    ...
    // the cache hit, then the 5th consecutive failure (total phrases 0
    // through 5) hits the limit and the generator returns early.
    isGPUReadyMock.mockReturnValue(true);
    ...
    synthesizeGPUMock.mockImplementation((text: string) => {
      if (text === "cached mid-run") {
        // Shouldn't be called — startSynth short-circuits on hasCachedAudio.
